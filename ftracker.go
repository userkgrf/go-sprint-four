...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// ваш код здесь
	v := meanSpeed(action, duration) // средняя скорость в км/ч
	return (runningCaloriesMeanSpeedMultiplier * v * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	v := meanSpeed(action, duration)
	return (walkingCaloriesWeightMultiplier*weight + (math.Pow(v*kmhInMsec, 2)/(height/cmInM))*
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	v := swimmingMeanSpeed(lengthPool, countPool, duration)
	return (v + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
}
